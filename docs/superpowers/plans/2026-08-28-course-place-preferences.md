...
- [x] 두 worktree의 브랜치와 working tree 상태를 확인한다.
- [x] 백엔드 테스트와 프론트엔드 테스트·빌드를 기준 상태에서 실행한다.
- [x] 기준 테스트 실패가 있으면 원인과 범위를 기록하고, 이번 변경으로 발생한 실패와 구분한다.
- [x] 설계 문서와 이 실행 계획을 백엔드 worktree에 커밋한다.
...
- Backend: bash gradlew test
- Frontend: npm test -- --run
- Frontend: npm run build

## Task 2: Persist detailed course preferences in the backend

- [x] CreateCourseRequest에 detailTypes를 추가하고, 입력값이 없거나 비어 있는 기존 요청은 허용한다.
- [x] Course 엔티티에 detailTypes 저장 필드와 getter를 추가한다.
- [x] 기존 생성자와 새 생성자가 모두 동작하도록 기존 생성자를 새 생성자에 위임한다.
- [x] CourseService가 요청의 detailTypes를 엔티티에 전달하도록 수정한다.
- [x] CourseResponse와 변환 로직에 detailTypes를 포함한다.
- [x] 허용된 세부 선호 ID와 현재 선택한 broad type의 조합을 검증하고, 알 수 없는 ID는 기존 프로젝트의 400 오류 규칙으로 반환한다.
- [x] CoursePreferenceTest와 CourseServiceTest에 저장·응답·하위 호환·잘못된 입력 테스트를 추가한다.
...
- [x] Scorer 테스트에 세부 선호가 정확히 일치하는 장소가 더 가까운 불일치 장소보다 우선되는 사례를 추가한다.
- [x] 음식 세부 타입, 휴식 세부 타입, 문화 세부 타입, 자연 타입의 키워드 표를 테스트로 고정한다.
- [x] 동행 유형 점수는 보조 지표로만 반영하고, 한 장소에서 여러 키워드가 맞아도 동행 점수 상한을 넘지 않도록 테스트한다.
- [x] 장소명·카테고리명·주소·도로명 주소를 검색 텍스트로 사용하도록 테스트한다.
- [x] 카페 주변 추천에서 메가MGC커피·메가커피·컴포즈커피·스타벅스·투썸플레이스 등 프랜차이즈 토큰이 있으면 감점하는 테스트를 추가한다.
- [x] Scorer의 점수 비중을 세부·broad 적합도 60, 거리 20, 동행 10, 데이터 완성도 10으로 변경한다.
- [x] 세부 일치 60, broad 키워드 일치 40, 카테고리만 일치 10, 부적합 0의 적합도 단계와 프랜차이즈 -8 감점을 구현한다.
- [x] 선택된 broad type이 둘 이상이어도 점수를 평균내지 않고 장소에 가장 잘 맞는 프로필의 점수를 사용한다.
- [x] 기존 active 데이터가 들어오면 기존 broad 규칙으로 안전하게 처리한다.
...
- [x] CoursePlaceService가 course의 detailTypes를 scorer에 전달하도록 수정한다.
- [x] 전체 검색 허용 카테고리에 attraction을 추가하고, 카테고리별 Kakao code 매핑을 유지한다.
- [x] all 검색에 keyword가 비어 있으면 Kakao API를 호출하지 않고 빈 결과와 종료 상태를 반환한다.
- [x] keyword가 있으면 선택 카테고리만 대상으로 Kakao keyword 검색을 호출하고, 기존 장소 중복 제거 규칙을 유지한다.
- [x] nearby의 stopId, 추천순·거리순, 2km 범위, 페이지네이션 동작은 유지한다.
- [x] representative는 프론트에서 API scope로 전송하지 않고 준비 중 상태로 처리한다.
- [x] CoursePlaceServiceTest에 attraction, blank all search no-call, submitted keyword search, detailTypes 전달 테스트를 추가한다.
- [x] API 계약 문서와 프로젝트 상태 문서에서 실제 지원 범위와 검색 전제 조건을 갱신한다.
...
- [x] 여행 타입 카탈로그에 확정된 세부 선호를 추가한다.
- [x] active를 새 기본 선택값으로 사용하지 않도록 하고, 기존 세션에 active만 남아 있어도 안전하게 읽는다.
- [x] Zustand store에 detailTypes와 세부 선호 토글 action을 추가하고 세션 저장 대상에 포함한다.
- [x] 타입과 세부 선호의 연결 관계를 검증하여 broad type을 해제하면 해당 detailTypes를 정리한다.
- [x] CourseOptions에서 선택된 broad type 아래에 해당 세부 선호 칩을 표시한다.
- [x] 코스 생성 요청에 detailTypes를 포함하고, 코스 응답에서 detailTypes를 읽도록 타입과 매핑을 수정한다.
- [x] CourseOptions와 course API 매핑 테스트를 추가한다.
...
- [x] NearbyPlaceCategory에 attraction을 추가한다.
- [x] UI용 장소 추가 모드를 nearby, all, representative로 정의한다. API scope에는 representative를 전송하지 않는다.
- [x] 기존에 평면으로 노출되던 3개 scope와 4개 category 버튼을 제거한다.
- [x] 첫 번째 줄에는 카페·음식점·문화시설·관광명소만 표시한다.
- [x] 선택된 카테고리 안에 주변 추천·강릉 전체 검색·강릉 대표를 segmented/in-option 형태로 표시한다.
- [x] 대표 모드는 비활성 또는 준비 중 상태로 보여주고 조회 요청을 만들지 않는다.
- [x] 주변 추천 모드에서는 선택한 관광지 최대 3개 중 하나를 기준으로 선택할 수 있게 유지하고, 추천순·거리순을 유지한다.
- [x] 강릉 전체 검색 모드에서는 초기 진입 시 빈 상태와 검색 안내만 보여준다.
- [x] 검색어 입력값과 제출값을 분리하고, 검색 버튼 또는 Enter 제출 전에는 React Query가 실행되지 않게 한다.
- [x] 카테고리·모드 변경 시 이전 제출 검색어와 결과를 정리한다.
- [x] 전체 검색 카드에는 거리와 추천 점수를 표시하지 않고, 주변 추천 카드에는 기존 추천 점수를 표시한다.
- [x] 장소 추가 패널의 empty, loading, error, no-result 상태를 새 구조에 맞게 정리한다.
- [x] CourseResult와 CourseResultPage 테스트로 7개 flat option 미노출, category-first, blank no-fetch, submit fetch, representative no-fetch를 검증한다.
...
- [x] API 계약에 detailTypes, attraction, blank all search guard, search submission, representative 준비 중 상태를 반영한다.
- [x] 실행 방법과 로컬 확인 절차에는 변경이 없어 기존 README를 유지한다.
- [x] 백엔드와 프론트엔드 diff에서 mock 데이터, 임시 로그, 사용하지 않는 import, dead state를 제거한다.
- [x] 두 worktree에서 git diff --check를 실행한다.
- [x] 백엔드 bash gradle test를 실행한다.
- [x] 프론트엔드 npm test -- --run과 npm run build를 실행한다.
- [x] 실제 API key가 없는 환경에서도 representative나 blank all search가 외부 API를 호출하지 않는지 테스트 결과로 확인한다.
- [x] 변경 요약, 검증 결과, 남은 비범위 항목을 작업 보고서에 기록한다.
