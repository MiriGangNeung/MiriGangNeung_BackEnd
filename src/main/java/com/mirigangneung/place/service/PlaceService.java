...
@Service public class PlaceService {
 ...
 @Transactional(readOnly=true) public PlaceDetailResponse detail(String id){Place p=find(id);return PlaceDetailResponse.from(p,images.findByPlaceOrderBySortOrderAsc(p).stream().map(PlaceImage::getImageUrl).toList());}
 @Transactional public List<PlaceResponse> candidates(String id,boolean related){Place p=find(id); List<TourApiClient.TourPlace> rs=related?tour.related(p.getTourContentId()):tour.search(p.getCategory(),p.getName(),0,20); return rs.stream().map(this::upsert).map(PlaceResponse::from).toList();}
